...
namespace Plane.Gameplay
{
    public class PlayerPlane : MonoBehaviour
    {
        ...
        void Start()
        {
            m_Joystick = GameObject.FindGameObjectWithTag("Joystick").GetComponent<Joystick>();
        }
        ...
        void Update()
        {
            ...
            if (Input.GetKey(KeyCode.LeftArrow) || m_Joystick.Horizontal < -0.2f)
            {
                InputX = -1;
            }
            else if (Input.GetKey(KeyCode.RightArrow) || m_Joystick.Horizontal > 0.2f)
            {
                InputX = 1;
            }

            if (Input.GetKey(KeyCode.UpArrow) || m_Joystick.Vertical > 0.2f)
            {
                InputY = 1;
            }
            else if (Input.GetKey(KeyCode.DownArrow) || m_Joystick.Vertical < -0.2f)
            {
                InputY = -1;
            }
